...
// Building Analysis Tabs Component
function BuildingAnalysisTabs({ agentData }) {
  ...
  const tabs = [
    ...
    { id: 'valuation', label: 'Value', icon: Wallet },
    { id: 'spatial', label: '3D Analysis', icon: Layers },
  ...
  return (
    <div className="space-y-1.5">
      {/* Header with tabs */}
      <div className="bg-slate-800/40 rounded-lg border border-cyan-500/30 overflow-hidden">
        <div className="flex items-center justify-between px-2 py-1.5 border-b border-slate-700/50">
          <h4 className="text-cyan-400 font-bold text-[11px] flex items-center gap-1.5 uppercase tracking-tight">
            <Building2 className="w-3.5 h-3.5" /> Building Analysis
          </h4>
...
3D ANALYSIS
-----------
Shadow Length: ${analysis.analysis_3d?.shadow_analysis?.shadow_length_m || 'N/A'}m
View Score: ${analysis.analysis_3d?.view_quality?.view_score || 'N/A'}/100
Best Floor: ${analysis.analysis_3d?.view_quality?.best_floor || 'N/A'}
Solar Potential: ${analysis.analysis_3d?.solar_potential?.potential_kw || 'N/A'} kW
Investment Score: ${analysis.analysis_3d?.investment_score?.overall_score || 'N/A'}/100
...
Report generated by Valora AI - City Intelligence Platform
        ...
        {/* Tab buttons */}
        <div className="flex items-center gap-0.5 px-1 py-1 bg-slate-900/30">
          {tabs.map(tab => (
            <button
              key={tab.id}
        ...
        <div className="p-2">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 text-[10px] flex-wrap">
                <div className="flex items-center gap-1 px-1.5 py-0.5 bg-slate-700/30 rounded">
                  ...
                  <span className="text-white font-bold">{analysis.building?.height || '?'}m</span>
                </div>
                <div className="flex items-center gap-1 px-1.5 py-0.5 bg-slate-700/30 rounded">
                  <span className="text-slate-500">F:</span>
                  <span className="text-white font-bold">{analysis.building?.levels || '?'}</span>
                </div>
                <div className="flex items-center gap-1 px-1.5 py-0.5 bg-cyan-500/10 rounded">
                  <span className="text-cyan-400 font-bold capitalize">{analysis.building?.type || '?'}</span>
                </div>
                <div className="flex items-center gap-1 px-1.5 py-0.5 bg-slate-700/30 rounded">
                  <span className="text-slate-500">A:</span>
                  <span className="text-white font-bold">{analysis.building?.area || '?'}m²</span>
                </div>
              </div>
              
              {/* Area Importance */}
              {analysis.area_importance && (
                  ...
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`px-1.5 py-0.5 rounded text-[9px] font-black ${
                        analysis.area_importance.grade === 'A+' ? 'bg-green-500/20 text-green-400' :
                        analysis.area_importance.grade === 'A' ? 'bg-blue-500/20 text-blue-400' :
                        analysis.area_importance.grade === 'B+' ? 'bg-cyan-500/20 text-cyan-400' :
                        'bg-yellow-500/20 text-yellow-400'
                      }`}>
                        {analysis.area_importance.grade}
                      </span>
                    </div>
                    <span className="text-white font-black text-sm">{analysis.area_importance.score}<span className="text-slate-500 text-[9px] font-normal">/100</span></span>
                  </div>
                  <div className="h-1 bg-slate-700/50 rounded-full overflow-hidden mb-2">
                    <div className="h-full bg-gradient-to-r from-blue-500 via-cyan-400 to-green-400 rounded-full" style={{ width: `${analysis.area_importance.score}%` }} />
                  </div>
                  <div className="flex items-center gap-3 text-[9px]">
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-0.5">
                        <span className="text-slate-500">Access</span>
                        <span className="text-white font-bold">{analysis.area_importance.factors?.accessibility || 0}</span>
                      </div>
                      <div className="h-1 bg-slate-700/50 rounded-full overflow-hidden">
                        <div className="h-full bg-green-500 rounded-full" style={{ width: `${analysis.area_importance.factors?.accessibility || 0}%` }} />
                      </div>
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-0.5">
                        <span className="text-slate-500">Walk</span>
                        <span className="text-white font-bold">{analysis.area_importance.factors?.walkability || 0}</span>
                      </div>
                      <div className="h-1 bg-slate-700/50 rounded-full overflow-hidden">
                        <div className="h-full bg-cyan-500 rounded-full" style={{ width: `${analysis.area_importance.factors?.walkability || 0}%` }} />
                      </div>
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-0.5">
                        <span className="text-slate-500">Amenity</span>
                        <span className="text-white font-bold">{Math.min(100, Math.round((analysis.area_importance.factors?.amenity_density || 0) / 2))}</span>
                      </div>
                      <div className="h-1 bg-slate-700/50 rounded-full overflow-hidden">
                        <div className="h-full bg-purple-500 rounded-full" style={{ width: `${Math.min(100, (analysis.area_importance.factors?.amenity_density || 0) / 2)}%` }} />
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {analysis.market && (
                <div className="flex items-center gap-2 text-[9px]">
                  <div className="flex items-center gap-1 px-1.5 py-0.5 bg-slate-700/30 rounded">
                    ...
                    <span className="text-white font-bold">{analysis.market.avg_price_per_sqft?.toLocaleString()}</span>
                    <span className="text-slate-500">/sqft</span>
                  </div>
                  {analysis.market.growth_1y && (
                    <div className="flex items-center gap-1 px-1.5 py-0.5 bg-green-500/10 rounded">
                      <span className="text-green-400 font-black">+{analysis.market.growth_1y}%</span>
                    </div>
                  )}
                  {analysis.market.demand_index && (
                    <div className={`px-1.5 py-0.5 rounded font-black text-[8px] uppercase ${
                      analysis.market.demand_index === 'High' ? 'bg-green-500/20 text-green-400' :
                      analysis.market.demand_index === 'Medium' ? 'bg-yellow-500/20 text-yellow-400' :
              ...
              {analysis.recommendations?.length > 0 && (
                <div className="bg-slate-800/40 rounded-lg p-2 border border-slate-700/50">
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span className="text-white font-bold text-[10px] uppercase">Key Highlights</span>
                  </div>
                  <div className="space-y-1">
                    {analysis.recommendations.slice(0, 3).map((rec, idx) => (
                      <div key={idx} className="flex items-start gap-2 text-[10px] text-slate-300 leading-tight">
          ...
          {activeTab === 'valuation' && (
            <div className="space-y-2">
              {analysis.valuation ? (
                  ...
                  <div className="bg-slate-800/40 rounded-lg p-3 border border-green-500/30">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-green-400 font-bold text-xs uppercase">Estimated Value</span>
                      </div>
                      <span className="text-slate-500 text-[9px] flex items-center gap-0.5">
                        <CheckCircle className="w-2.5 h-2.5" />{(analysis.valuation.confidence * 100).toFixed(0)}% confidence
                      </span>
                    </div>
                    <div className="text-green-400 font-black text-3xl mb-2">
                      ₹{(analysis.valuation.estimated_price / 100000).toFixed(1)}L
                    </div>
                    <div className="text-slate-400 text-xs mb-3">
                      ₹{analysis.valuation.price_per_sqft?.toLocaleString()}/sqft
                    </div>
                    {/* Price Range Visual */}
                    <div className="relative h-6 bg-slate-700/30 rounded-full overflow-hidden">
                      ...
                      <div className="absolute inset-0 flex items-center justify-between px-3 text-[9px]">
                        <span className="text-slate-400 font-medium">₹{(analysis.valuation.price_range?.low / 100000).toFixed(0)}L</span>
                        <span className="text-slate-400 font-medium">₹{(analysis.valuation.price_range?.high / 100000).toFixed(0)}L</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {analysis.market && (
                    <div className="bg-slate-800/40 rounded-lg p-2 border border-slate-700/50">
                      <div className="flex items-center gap-1.5 mb-1.5">
                        ...
                        <span className="text-blue-400 font-bold text-[10px] uppercase">Market Context</span>
                      </div>
                      <div className="grid grid-cols-2 gap-2 text-[10px]">
                        <div className="bg-slate-700/30 rounded p-1.5">
                          <div className="text-slate-500">Avg Price</div>
                          <div className="text-white font-bold">₹{analysis.market.avg_price_per_sqft?.toLocaleString()}/sqft</div>
                        </div>
                        <div className="bg-slate-700/30 rounded p-1.5">
                          <div className="text-slate-500">1Y Growth</div>
                          <div className="text-green-400 font-bold">+{analysis.market.growth_1y}%</div>
                        </div>
                        <div className="bg-slate-700/30 rounded p-1.5">
                          <div className="text-slate-500">Properties</div>
                          <div className="text-white font-bold">{analysis.market.total_properties || 0}</div>
                        </div>
                        <div className="bg-slate-700/30 rounded p-1.5">
                          <div className="text-slate-500">Demand</div>
                          <div className={`font-bold ${analysis.market.demand_index === 'High' ? 'text-green-400' : analysis.market.demand_index === 'Medium' ? 'text-yellow-400' : 'text-red-400'}`}>
                            {analysis.market.demand_index}
                          </div>
                        </div>
                      </div>
                ...
                <div className="text-slate-400 text-xs text-center py-4">No valuation data available</div>
              )}
            </div>
          )}
          ...
          {activeTab === 'spatial' && (
            <div className="space-y-2">
              {analysis.analysis_3d ? (
                <>
                  {/* Shadow Analysis */}
                  {analysis.analysis_3d.shadow_analysis && (
                    <div className="bg-slate-800/40 rounded-lg p-2 border border-slate-700/50">
                      <div className="flex items-center gap-1.5 mb-1.5">
                        <span className="text-lg">🌑</span>
                        <span className="text-white font-bold text-[10px] uppercase">Shadow Analysis</span>
                      </div>
                      <div className="grid grid-cols-2 gap-2 text-[10px]">
                        <div>
                          <span className="text-slate-500">Length:</span>
                          <span className="text-white font-bold ml-1">{analysis.analysis_3d.shadow_analysis.shadow_length_m}m</span>
                        </div>
                        <div>
                          <span className="text-slate-500">Direction:</span>
                          <span className="text-white font-bold ml-1">{analysis.analysis_3d.shadow_analysis.shadow_direction}</span>
                        </div>
                        <div>
                          <span className="text-slate-500">Hours/Day:</span>
                          <span className="text-white font-bold ml-1">{analysis.analysis_3d.shadow_analysis.shadow_hours_per_day}h</span>
                        </div>
                        <div>
                          <span className="text-slate-500">Impact:</span>
                          <span className={`font-bold ml-1 ${analysis.analysis_3d.shadow_analysis.impact_level === 'low' ? 'text-green-400' : 'text-yellow-400'}`}>
                            {analysis.analysis_3d.shadow_analysis.impact_level}
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  {/* View Quality */}
                  {analysis.analysis_3d.view_quality && (
                    <div className="bg-slate-800/40 rounded-lg p-2 border border-slate-700/50">
                      <div className="flex items-center gap-1.5 mb-1.5">
                        <Eye className="w-3 h-3 text-blue-400" />
                        <span className="text-white font-bold text-[10px] uppercase">View Quality</span>
                        <span className="ml-auto text-white font-black text-sm">{analysis.analysis_3d.view_quality.view_score}<span className="text-slate-500 text-[9px]">/100</span></span>
                      </div>
                      <div className="h-1.5 bg-slate-700/50 rounded-full overflow-hidden mb-2">
                        <div className="h-full bg-gradient-to-r from-blue-500 to-cyan-400 rounded-full" style={{ width: `${analysis.analysis_3d.view_quality.view_score}%` }} />
                      </div>
                      <div className="grid grid-cols-2 gap-2 text-[10px]">
                        <div>
                          <span className="text-slate-500">Best Floor:</span>
                          <span className="text-green-400 font-bold ml-1">F{analysis.analysis_3d.view_quality.best_floor}</span>
                        </div>
                        <div>
                          <span className="text-slate-500">Worst Floor:</span>
                          <span className="text-red-400 font-bold ml-1">F{analysis.analysis_3d.view_quality.worst_floor}</span>
                        </div>
                        <div className="col-span-2">
                          <span className="text-slate-500">Open Directions:</span>
                          <span className="text-white font-bold ml-1">{analysis.analysis_3d.view_quality.open_directions?.join(', ')}</span>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {/* 3D Neighbors */}
                  {analysis.analysis_3d.neighbors_3d && (
                    <div className="bg-slate-800/40 rounded-lg p-2 border border-slate-700/50">
                      <div className="flex items-center gap-1.5 mb-1.5">
                        <Building2 className="w-3 h-3 text-purple-400" />
                        <span className="text-white font-bold text-[10px] uppercase">3D Neighbors</span>
                      </div>
                      <div className="grid grid-cols-3 gap-2 text-[10px] mb-2">
                        <div className="text-center bg-slate-700/30 rounded p-1">
                          <div className="text-slate-500 text-[8px]">Above</div>
                          <div className="text-white font-bold">{analysis.analysis_3d.neighbors_3d.buildings_above}</div>
                        </div>
                        <div className="text-center bg-slate-700/30 rounded p-1">
                          <div className="text-slate-500 text-[8px]">At Level</div>
                          <div className="text-white font-bold">{analysis.analysis_3d.neighbors_3d.buildings_at_level}</div>
                        </div>
                        <div className="text-center bg-slate-700/30 rounded p-1">
                          <div className="text-slate-500 text-[8px]">Below</div>
                          <div className="text-white font-bold">{analysis.analysis_3d.neighbors_3d.buildings_below}</div>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 text-[10px]">
                        <div>
                          <span className="text-slate-500">Privacy:</span>
                          <span className="text-white font-bold ml-1">{analysis.analysis_3d.neighbors_3d.privacy_score}%</span>
                        </div>
                        <div>
                          <span className="text-slate-500">Light:</span>
                          <span className="text-white font-bold ml-1">{analysis.analysis_3d.neighbors_3d.light_access_score}%</span>
                        </div>
                      </div>
                    </div>
                  )}
                ...
                <div className="text-slate-400 text-xs text-center py-4">No 3D analysis available</div>
              )}
            </div>
          )}
          ...
          {activeTab === 'solar' && (
            <div className="space-y-2">
              {analysis.analysis_3d?.solar_potential ? (
                <div className="bg-gradient-to-br from-yellow-500/10 to-orange-500/10 rounded-lg p-3 border border-yellow-500/30">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div>
                      ...
                      <div className="text-yellow-400 text-xs capitalize">{analysis.analysis_3d.solar_potential.suitability} suitability</div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <div className="bg-slate-800/50 rounded p-2">
                      ...
                      <div className="text-white font-bold text-lg">{analysis.analysis_3d.solar_potential.roof_area_sqm}m²</div>
                    </div>
                    <div className="bg-slate-800/50 rounded p-2">
                      <div className="text-slate-400 text-[10px]">Peak Sun Hours</div>
                      <div className="text-white font-bold text-lg">{analysis.analysis_3d.solar_potential.solar_hours}h</div>
                    </div>
                  </div>
                  
                  <div className="bg-slate-800/50 rounded p-3 text-center">
                    <div className="text-slate-400 text-[10px] mb-1">Potential Capacity</div>
                    <div className="text-yellow-400 font-black text-2xl">{analysis.analysis_3d.solar_potential.potential_kw} kW</div>
                    <div className="text-slate-500 text-[9px]">~{Math.round(analysis.analysis_3d.solar_potential.potential_kw * 4)} units/day</div>
                  </div>
                  
                  <div className="mt-3 text-[10px] text-slate-400">
                    💡 Rooftop solar can reduce electricity bills by 70-90% with {analysis.analysis_3d.solar_potential.potential_kw}kW system
                  </div>
                </div>
              ) : (
                <div className="text-slate-400 text-xs text-center py-4">No solar analysis available</div>
              )}
            </div>
          )}
          ...
          {activeTab === 'investment' && (
            <div className="space-y-2">
              {analysis.analysis_3d?.investment_score ? (
                <>
                  <div className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 rounded-lg p-3 border border-green-500/30">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-white font-bold">Investment Score</span>
                      </div>
                      <span className="text-green-400 font-black text-3xl">{analysis.analysis_3d.investment_score.overall_score}<span className="text-slate-500 text-sm">/100</span></span>
                    </div>
                    <div className="h-2 bg-slate-700/50 rounded-full overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-yellow-500 via-green-500 to-emerald-400 rounded-full" style={{ width: `${analysis.analysis_3d.investment_score.overall_score}%` }} />
                    </div>
                  </div>
                  
                  <div className="grid grid-cols-2 gap-2">
                    <div className="bg-slate-800/40 rounded-lg p-2 border border-slate-700/50">
                      <div className="text-slate-500 text-[10px] mb-1">Appreciation</div>
                      <div className={`font-bold text-sm ${analysis.analysis_3d.investment_score.appreciation_potential === 'high' ? 'text-green-400' : 'text-yellow-400'}`}>
                        {analysis.analysis_3d.investment_score.appreciation_potential?.toUpperCase()}
                      </div>
                    </div>
                    <div className="bg-slate-800/40 rounded-lg p-2 border border-slate-700/50">
                      <div className="text-slate-500 text-[10px] mb-1">Rental Yield</div>
                      <div className="text-white font-bold text-sm">{analysis.analysis_3d.investment_score.rental_yield_estimate}</div>
                    </div>
                  </div>
                  
                  {analysis.analysis_3d.investment_score.key_factors && (
                    <div className="bg-slate-800/40 rounded-lg p-2 border border-slate-700/50">
                      <div className="text-slate-500 text-[10px] mb-1.5">Key Factors</div>
                      <div className="flex flex-wrap gap-1">
                        {analysis.analysis_3d.investment_score.key_factors.map((factor, idx) => (
                  ...
                  {analysis.ai_analysis && (
                    <div className="bg-purple-600/5 border border-purple-500/20 rounded-lg p-3">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-purple-400 font-bold text-xs uppercase">AI Analysis</span>
                      </div>
                ...
                <div className="text-slate-400 text-xs text-center py-4">No investment analysis available</div>
              )}
            </div>
          )}
        </div>
...
export default function AnalysisPanel({ agentData, setAgentData, activeTab, setActiveTab, fontSize = 100, liveAnalysis = null, isFullscreen = false, userTier = 'free' }) {
        ...
        {activeTab === 'insights' && (
          <div className="space-y-2">
            ...
            {agentData?.buildingAnalysis && (
              <BuildingAnalysisTabs agentData={agentData} />
            )}
